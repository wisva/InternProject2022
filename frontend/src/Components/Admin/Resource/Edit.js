...
function Add() {
  ...
  const particularData = useSelector(
    (state) => state.resourceAdminParticularList.resource
  );
  const { id } = useParams();
  console.log(id, "id");
  ...
  const handleCancelClick = () => {
    dispatch(resourceEdit(id, name));
  };

  useEffect(() => {
    dispatch(resourceParticularGet(id));
  }, []);
